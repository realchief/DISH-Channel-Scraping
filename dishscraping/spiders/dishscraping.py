...
try:
    with open(os.path.abspath('DISH_input.csv'), 'r') as f:
        reader = csv.reader(f)
        for row in reader:
...
class DishroductsSpider (scrapy.Spider):
    name = "dish_products"
    allowed_domains = ['dish.com']

    start_urls = ['https://www.dish.com/programming/channels/']
